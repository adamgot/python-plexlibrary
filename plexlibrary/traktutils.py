...
class Trakt(object):
    ...
    def add_movies(url, movie_list=None, movie_ids=None):
        if not movie_list:
            movie_list = []
        if not movie_ids:
            movie_ids = []
    ...
    def add_shows(url, show_list=None, show_ids=None):
        if not show_list:
            show_list = []
        if not show_ids:
            show_ids = []
    ...
    def add_items(item_type, url, item_list=None, item_ids=None):
        if item_type == 'movie':
